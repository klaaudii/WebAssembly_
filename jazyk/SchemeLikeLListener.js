// Generated from SchemeLikeL.g4 by ANTLR 4.13.1
// jshint ignore: start
import antlr4 from './antlr4/index.web.js';

// This class defines a complete listener for a parse tree produced by SchemeLikeLParser.
export default class SchemeLikeLListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by SchemeLikeLParser#start.
	enterStart(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#start.
	exitStart(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#sExpr.
	enterSExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#sExpr.
	exitSExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#expr.
	enterExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#expr.
	exitExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#logExpr.
	enterLogExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#logExpr.
	exitLogExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#defineGlobalVar.
	enterDefineGlobalVar(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#defineGlobalVar.
	exitDefineGlobalVar(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#defineFnc.
	enterDefineFnc(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#defineFnc.
	exitDefineFnc(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#fncBodyExpr.
	enterFncBodyExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#fncBodyExpr.
	exitFncBodyExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#localFncBodyExpr.
	enterLocalFncBodyExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#localFncBodyExpr.
	exitLocalFncBodyExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#varPairExpr.
	enterVarPairExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#varPairExpr.
	exitVarPairExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#beginExpr.
	enterBeginExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#beginExpr.
	exitBeginExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#biExpr.
	enterBiExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#biExpr.
	exitBiExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#logBiExpr.
	enterLogBiExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#logBiExpr.
	exitLogBiExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#uniExpr.
	enterUniExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#uniExpr.
	exitUniExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#logUniExpr.
	enterLogUniExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#logUniExpr.
	exitLogUniExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#callFnc.
	enterCallFnc(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#callFnc.
	exitCallFnc(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#callLambdaFnc.
	enterCallLambdaFnc(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#callLambdaFnc.
	exitCallLambdaFnc(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#ifExpr.
	enterIfExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#ifExpr.
	exitIfExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#operatorExpr.
	enterOperatorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#operatorExpr.
	exitOperatorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#logOperatorExpr.
	enterLogOperatorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#logOperatorExpr.
	exitLogOperatorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#biOperatorExpr.
	enterBiOperatorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#biOperatorExpr.
	exitBiOperatorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#logBiOperatorExpr.
	enterLogBiOperatorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#logBiOperatorExpr.
	exitLogBiOperatorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#uniOperatorExpr.
	enterUniOperatorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#uniOperatorExpr.
	exitUniOperatorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#logUniOperatorExpr.
	enterLogUniOperatorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#logUniOperatorExpr.
	exitLogUniOperatorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#setExpr.
	enterSetExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#setExpr.
	exitSetExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#vectorExpr.
	enterVectorExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#vectorExpr.
	exitVectorExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#listExpr.
	enterListExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#listExpr.
	exitListExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#displayExpr.
	enterDisplayExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#displayExpr.
	exitDisplayExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#exportExpr.
	enterExportExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#exportExpr.
	exitExportExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#whileExpr.
	enterWhileExpr(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#whileExpr.
	exitWhileExpr(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#literal.
	enterLiteral(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#literal.
	exitLiteral(ctx) {
	}


	// Enter a parse tree produced by SchemeLikeLParser#identifier.
	enterIdentifier(ctx) {
	}

	// Exit a parse tree produced by SchemeLikeLParser#identifier.
	exitIdentifier(ctx) {
	}



}